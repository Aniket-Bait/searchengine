...
def start(url):
    ...
    for each_text in soup.findAll('div'):
        anchor_tag = each_text.a
        links = ''
        if anchor_tag is not None:
            try:
                links = each_text.a['href']
            except KeyError:
                pass
        if "https" not in links:
            print('url+links: ', links)
            anchor_source_code = requests.get(url+links).text
        else:
            print('links: ', links)
            anchor_source_code = requests.get(links).text
        soup_a = BeautifulSoup(anchor_source_code, 'html.parser')
        content = ''
        for i in soup_a.findAll():
            content = i.text
